...
class Correlation:
    ...
    def correlateCurves(self):
        ...
        if delta_t_start and self.data_point_1.spectrum_data.end > self.data_point_2.spectrum_data.end:
            curve1 = curve1[:-int(self.data_per_second_1 * abs(delta_t_end))]
        else:
            curve2 = curve2[:-int(self.data_per_second_2 * abs(delta_t_end))]

        if delta_t_end and self.data_per_second_1 < self.data_per_second_2:
...
def setupSummedCurve(data_point: data.DataPoint, frequency_range, flatten, flatten_window):
